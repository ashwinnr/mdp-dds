...
do 
	...
	do
		for maxIgnore in 10 
		do
			JOB_NAME=skill_teaching_incrSymRTDP_$nobj\_10000_$timeOut\_$maxIgnore
			cp run_incr_srtdp.sh /tmp/academic/$JOB_NAME.sh
			sed -i "s/DOMAIN_FILENAME/\/nfs\/stak\/students\/n\/nadamuna\/mdp-dds\/rddl\/skill_teaching_mdp.rddl/" /tmp/academic/$JOB_NAME.sh
			sed -i "s/NUM_STATE/$NUM_STATE/"  /tmp/academic/$JOB_NAME.sh
			sed -i "s/NUM_ROUNDS/$NUM_ROUNDS/"   /tmp/academic/$JOB_NAME.sh
			sed -i "s/INSTANCE_FILENAME/\/nfs\/stak\/students\/n\/nadamuna\/mdp-dds\/rddl\/skill_teaching_inst_mdp__$nobj.rddl/"   /tmp/academic/$JOB_NAME.sh
			...
			sed -i "s/INITIALIZATION/CONCRETE_STATE/" /tmp/academic/$JOB_NAME.sh
			sed -i "s/STOPPING/TIMEOUT/" /tmp/academic/$JOB_NAME.sh
			sed -i "s/ELIMINATION/FROM_LGG/" /tmp/academic/$JOB_NAME.sh
			sed -i "s/IGNORE_CAP/$maxIgnore/" /tmp/academic/$JOB_NAME.sh
			sed -i "s/STATETIME/-1/" /tmp/academic/$JOB_NAME.sh
			echo $JOB_NAME 
