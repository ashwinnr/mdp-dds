...
public class RDDL2Graph {
	...
	public static final int DEFAULT_VIEWER_TEXT_LINES = 10;
	...
	public Graph rddl2graph() throws Exception {
		...
		HashSet<UnorderedPair< PVAR_NAME, ArrayList<LCONST>>> parents 
		...
		for (Map.Entry<Pair, PVAR_NAME> e : _state._tmIntermNames.entrySet()) {
			...
			for (ArrayList<LCONST> gfluent : gfluents) {
				...
				for (UnorderedPair< PVAR_NAME, ArrayList<LCONST>> par : parents){
		...
		// Create reward node
//		rewards = getAdditiveComponents(_d._exprReward);
	...
	private ArrayList<UnorderedPair> getAdditiveComponents(EXPR e) throws Exception {

		ArrayList<UnorderedPair> ret = new ArrayList<UnorderedPair>();
		...
		if (e instanceof OPER_EXPR && ((OPER_EXPR)e)._op == OPER_EXPR.PLUS) {
			...
			for (ArrayList<LCONST> sub_inst : possible_subs) {
				...
				ret.add(new UnorderedPair(subs.clone(), a._e));			
			...
			ret.add(new UnorderedPair(empty_subs, e));
		}
