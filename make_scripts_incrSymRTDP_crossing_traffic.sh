...
do 
	...
	do
		for lookahead in 16
		do
			for prune_limit in 1e3 1e4 
			do

			JOB_NAME=crossing_traffic_incr_srtdp_$prune_limit\_$nobj\_$timeOut\_$lookahead
			...
			sed -i "s/INSTANCE_FILENAME/\/nfs\/stak\/students\/n\/nadamuna\/mdp-dds\/rddl\/crossing_traffic_inst_mdp__$nobj.rddl/"   /tmp/academic/$JOB_NAME.sh
