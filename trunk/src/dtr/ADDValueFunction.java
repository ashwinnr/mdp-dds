...
public class ADDValueFunction 
	...
	public void addPermenant( ){
		...
		ADDRNode[] arr = new ADDRNode[1+( _jointQFn != null && _qFn == null ? 1 : 0 )
		                              	+ ( _jointQFn == null && _qFn != null ? size : 0 )];
		arr[0] = _valueFn;
		if( _jointQFn != null && _qFn == null ){
			arr[1] = _jointQFn;
		}else{
			int i = 1;
	...
	public <T extends FactoredState<RDDLFactoredStateSpace>> 
		...
		ADDRNode leaf = _manager.evaluate(_valueFn, factored_state);
	...
	public <T extends FactoredState<RDDLFactoredStateSpace>, U extends 
		...
		if( qFun == null ){
			qFun = ( _jointQFn == null ) ? null : _jointQFn;
		}
	...
	public void throwAwayQFunctions() {
		...
		if( _jointQFn != null ){
			_manager.removePermenant( _jointQFn );
			_jointQFn.nullify();
			_jointQFn = null;
		}
	}
