...
set eval_steps = STEPS_POLICY_EVALUATION
...
cp -f /nfs/stak/students/n/nadamuna/mdp-dds/dist/MPI.jar /tmp/$job_title\_tmp/MPI.jar
...
time /usr/local/common64/jdk1.7.0_45/bin/java -jar -Xmx4g -Xms1g -ea /tmp/$job_title\_tmp/MPI.jar /tmp/$job_title\_tmp/$job_title\_dom /tmp/$job_title\_tmp/$job_title\_inst 0.1 42 $disc 1 30 $eval_steps $far $naive_constraints $apricodd $apricodd_error $apricodd_type $time_out_Mins RDDL 1 >& /tmp/$job_title\_tmp/$job_title
